            ...
            <div class="row">
                <div class="col-md-12 col-lg-12 col-sm-12">
                    ...
                    <div class="white-box">
                        ...
                        <form class="row g-3" method="post" action ="{{ route('admin.storePostCategories')  }}">
                            @csrf
                            <div class="col-auto">
                                <input type="text" name="blog_category_name" class="form-control" id="category" placeholder="კატეგორია">
                        ...
                        <div class="table-responsive">
                            <table class="table no-wrap">
                                ...
                                @forelse ($blog_categories as $category)
                                    <tr>
                                        <td>{{ $category->id }}</td>
                                        <td><a href="{{ route('blog.wfilter',['category' => $category->url_name]) }}">{{ $category->blog_category_name }}</a></td>
                                        <td>
                                            <form action="{{ route('admin.deletePostCategory')  }}" method="post">
                                                @method('DELETE')
                                                @csrf
                                                <input type="hidden" name="category_id" value="{{ $category->id }}">
                                                <button class="btn btn-danger btn-sm text-white" type="submit" href="{{ route('admin.showPostCategories')  }}">წაშლა</button>
                                            </form>
